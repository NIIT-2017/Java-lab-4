...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int c = 0; c<cleaners.size(); c++) {
            float workedHours = (float) (Math.random() * (63+21));
        ...
        for (int d = 0; d<drivers.size(); d++) {
            float workedHours = (float) (Math.random() * (84+24));
        ...
        for (int tl = 0; tl<teamLeaders.size(); tl++){
            ...
            System.out.printf("%-32s%-30s%-30s%-30s%-30.2f%-30d%-30s%-10d%n", teamLeaders.get(tl).getFio() , teamLeaders.get(tl).getPosition(), "NO", "NO", teamLeaders.get(tl).getPayment(), 0, teamLeaders.get(tl).getProject(), (com.getNumberOfProgrammersInProject(teamLeaders.get(tl).getProject())+com.getNumberOfTestersInProject(teamLeaders.get(tl).getProject())));
            writer.printf("%-32s%-30s%-30s%-30s%-30.2f%-30d%-30s%-10d%n", teamLeaders.get(tl).getFio() , teamLeaders.get(tl).getPosition(), "NO", "NO", teamLeaders.get(tl).getPayment(), 0, teamLeaders.get(tl).getProject(), (com.getNumberOfProgrammersInProject(teamLeaders.get(tl).getProject())+com.getNumberOfTestersInProject(teamLeaders.get(tl).getProject())));
        }
        //Calculate month  salary for testers
        ArrayList<Tester> testers = com.getAllTesters();
        for (int t = 0; t<testers.size(); t++) {
            float workedHours = (float) (Math.random() * (168+84));
